...
public class BasicDetailScreen_User_Onboarding {
	...
	public void userOnbording_Positive() throws Exception {
		...
		ValidatableResponse response = Utilities.PlayStore_User_OnboardingAPI_Segment1(data);
...
//		Validation.assertEquals(response.extract().body().jsonPath().get("data.stage"),"basic_details_pending","userOnbording_Positive,Validating stage value");
//		Validation.assertEquals(response.extract().body().jsonPath().get("data.stage_detail"),"apply","userOnbording_Positive,Validating stage_detail");
